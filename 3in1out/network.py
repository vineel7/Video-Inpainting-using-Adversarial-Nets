...
class Network:
    def __init__(self, x, mask, points, local_x, is_training, batch_size,local_size):
        self.alpha = 1
        ...
        self.local_completionG = tf.reshape(self.local_completionG, [batch_size, local_size, local_size, 3])
        ...
        def global_discriminator(x):
            ...
            with tf.variable_scope('global'):
                ...
                with tf.variable_scope('conv5'):
                    ...
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
                with tf.variable_scope('conv6_new'):
                    x = conv_layer(x, [5, 5, 512, 512], 2)
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
        ...
        def local_discriminator(x):
            ...
            with tf.variable_scope('local'):
                ...
                with tf.variable_scope('conv4'):
                    ...
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
                with tf.variable_scope('conv5_new'):
                    x = conv_layer(x, [5, 5, 512, 512], 2)
                    x = batch_normalize(x, is_training)
                    x = tf.nn.relu(x)
                with tf.variable_scope('fc'):
