...
#================ completion part ============================
        self.completion_3d,self.imitation_3d = self.completionnetwork_3d(x, mask,is_training)
...
#=================  MC part ===========================================================================
        self.completion_3d_mc = tf.transpose(self.completion_3d,[0,2,3,4,1])
        #print('shape after mc for inpainted--'+str(self.completion_3d_mc.get_shape().as_list()))
        self.flow1,self.completion_3d_mc = self.model_mc(self.completion_3d_mc,reuse=False)
        #print('shape after mc for inpainted--'+str(self.completion_3d_mc.get_shape().as_list()))
        self.completion_3d_mc = tf.concat([tf.concat([self.completion_3d_mc[0:batch_size,:, :, :], x[:,2, :, :, :]], axis=0),
        				   self.completion_3d_mc[batch_size:2*batch_size,:, :, :]],axis=0)
        ...
        self.completion_3d_mc_reshape = tf.convert_to_tensor(self.completion_3d_mc_reshape)
        #print('shape after reshaping inpainted--'+str(self.completion_3d_mc_reshape.get_shape().as_list()))
        #print('=========================================')

        self.local_completionG_mc = tf.transpose(self.local_completionG,[0,2,3,4,1])
	#print('input shape to mc for local inpainted '+str(self.local_completionG_mc.get_shape().as_list()))
        self.flow2,self.local_completionG_mc = self.model_mc(self.local_completionG_mc,reuse=True)
	#print('shape after mc for local inpainted '+str(self.local_completionG_mc.get_shape().as_list()))
        self.local_completionG_mc = tf.concat([tf.concat([self.local_completionG_mc[0:batch_size,:, :, :],local_x[:,1,:,:,:]],axis=0),
					       self.local_completionG_mc[batch_size:2*batch_size,:, :, :]],axis=0)
	#print('shape after adding middle '+str(self.local_completionG_mc.get_shape().as_list()))
        ...
        self.local_completionG_mc_reshape = tf.convert_to_tensor(self.local_completionG_mc_reshape)
        #print('shape after reshaping '+str(self.local_completionG_mc_reshape.get_shape().as_list()))
	#print('========================================')

        self.real_glo_dis_mc = tf.transpose(x[:, 1:4, :, :, :],[0,2,3,4,1])
	#print('input to mc for real x--'+str(self.real_glo_dis_mc.get_shape().as_list()))
        self.flow3,self.real_glo_dis_mc = self.model_mc(self.real_glo_dis_mc,reuse=True)
	#print('output after mc for real x --'+str(self.real_glo_dis_mc.get_shape().as_list()))
        self.real_glo_dis_mc = tf.concat([tf.concat([self.real_glo_dis_mc[0:batch_size,:, :, :],x[:,2, :, :, :]], axis=0),
					  self.real_glo_dis_mc[batch_size:2*batch_size,:, :, :]],axis=0)
	#print('shape after adding middle--'+str(self.real_glo_dis_mc.get_shape().as_list()))
        ...
        self.real_glo_dis_mc_reshape = tf.convert_to_tensor(self.real_glo_dis_mc_reshape)
	...
	#print('input shape to mc for local real--'+str(self.real_loc_dis_mc.get_shape().as_list()))
        self.flow4,self.real_loc_dis_mc = self.model_mc(self.real_loc_dis_mc,reuse=True)
	#print('output after mc for local real--'+str(self.real_loc_dis_mc.get_shape().as_list()))
        self.real_loc_dis_mc = tf.concat([tf.concat([self.real_loc_dis_mc[0:batch_size,:, :, :],local_x[:,1, :, :, :]], axis=0),
                                          self.real_loc_dis_mc[batch_size:2*batch_size,:, :, :]],axis=0)
	#print('shape after adding middle--'+str(self.real_loc_dis_mc.get_shape().as_list()))
        ...
        self.real_loc_dis_mc_reshape = tf.convert_to_tensor(self.real_loc_dis_mc_reshape)
        #print('shape after reshaping--'+str(self.real_loc_dis_mc_reshape.get_shape().as_list()))

#============ discriminator part =================================

        self.real = self.discriminator(self.real_glo_dis_mc_reshape, self.real_loc_dis_mc_reshape, reuse=False)
        self.fake = self.discriminator(self.completion_3d_mc_reshape,self.local_completionG_mc_reshape, reuse=True)
    ...
    def completionnetwork_3d(self, x, mask,is_training):
        self.imitation_3d = []
        self.completion_3d = []
        for i in range(3):
                self.x_3d = x[:,i:i+3,:,:,:]
