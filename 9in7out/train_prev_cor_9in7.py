# 9in7 running on vidtimit with pretrained on celebA
import numpy as np
import tensorflow as tf
import cv2
import tqdm
from network_9in7 import Network
import load
import logging
...
logging.basicConfig(level=logging.DEBUG, filename='/home/avisek/vineel/glcic-master/9_in_7_video/lossvalues_prevcor.log',filemode='a+',format='%(asctime)s %(message)s')

dir1='/home/avisek/vineel/glcic-master/9_in_7_video/output_9in7_prevcor/'
dir2='/home/avisek/vineel/glcic-master/9_in_7_video/original_9in7_prevcor/'
dir3='/home/avisek/vineel/glcic-master/9_in_7_video/perturbed_9in7_prevcor/'
...
def train():
    ...
    local_x = tf.placeholder(tf.float32, [BATCH_SIZE, 7 , LOCAL_SIZE, LOCAL_SIZE, 3])
    is_training = tf.placeholder(tf.bool, [])
    points = tf.placeholder(tf.int32, [BATCH_SIZE, 7 , 4])

    model = Network(x, mask, points, local_x, is_training, batch_size=BATCH_SIZE)
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    #session = tf.Session(config=config, ...)
    #config.gpu_options.per_process_gpu_memory_fraction = 0.74
    ...
    if tf.train.get_checkpoint_state('/home/avisek/vineel/glcic-master/9_in_7_video/backup'):
        saver = tf.train.Saver()
        saver.restore(sess, '/home/avisek/vineel/glcic-master/9_in_7_video/backup/latest')
    ...
    while sess.run(epoch) < Tot_EPOCH:
        ...
        if sess.run(epoch) <= PRETRAIN_EPOCH:
            ...
            save_samples(dir2, cnt,gc_loss, x_batch[:, 1:8, :, :, :])
            save_samples(dir3, cnt,gc_loss, x_pert[:, 1:8, :, :, :])
            saver = tf.train.Saver()
        ...
        #=======================================    Discrimitation network - phase 2 ========================================================
            ...
            for i in tqdm.tqdm(range(step_num)):
                ...
                local_x_batch = local_x_batch.reshape(BATCH_SIZE, 7, LOCAL_SIZE, LOCAL_SIZE, 3)
...
def get_points2(x_batch):
    ...
    mask = []
    global x,y,w,h
    x,y,w,h=np.array([5,5,IMAGE_SIZE,IMAGE_SIZE])
    for i in range(BATCH_FRAME_SIZE):
...
def save_samples(dir,cnt,loss,x_batch):
	...
	for i in range(BATCH_SIZE):
		for j in range(frames_no):
			...
			cv2.imwrite(str(dir)+'inpainted_epoch_'+str(cnt)+'loss_'+str(loss)+'frame_%02d.jpg' % (3*i+j) ,(frame))
