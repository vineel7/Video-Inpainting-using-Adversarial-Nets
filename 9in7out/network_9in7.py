...
class Network:
    def __init__(self, x, mask, points, local_x, is_training, batch_size):
        ...
        self.real = self.discriminator(x[:, 1:8, :, :, :], local_x, reuse=False)
        self.fake = self.discriminator(self.completion_3d, self.local_completionG, reuse=True)
        self.g_loss = self.calc_g_loss(x[:, 1:8, :, :, :], self.completion_3d)/(self.batch_size*7)
    ...
    def local_completion(self,completion_3d,points):
        ...
        self.local_completionG = tf.reshape(self.local_completionG, [self.batch_size, 7, 64, 64, 3])
        ...
        def global_discriminator(x):
            ...
            with tf.variable_scope('global'):
                with tf.variable_scope('conv1'):
                    ...
                    #print(x.get_shape().as_list())
                with tf.variable_scope('conv2'):
                    x = conv3_layer1(x, [3, 5, 5, 64, 128], 2)
                    x = batch_normalize_3d(x, is_training)
                    x = tf.nn.relu(x)
                    #print('shape of global dis 2 layer out')
                    #print(x.get_shape().as_list())
                with tf.variable_scope('conv3'):
                    x = conv3_layer1(x, [3, 5, 5, 128, 256], 2)
                    x = batch_normalize_3d(x, is_training)
                    x = tf.nn.relu(x)
                    #print('shape of global dis 3 layer out')
                    #print(x.get_shape().as_list())
                with tf.variable_scope('conv4'):
                    x = conv3_layer1(x, [3, 5, 5, 256, 512], 2)
                    x = batch_normalize_3d(x, is_training)
                    x = tf.nn.relu(x)
                    #print('shape of global dis 4 layer out')
                    #print(x.get_shape().as_list())
                with tf.variable_scope('conv5'):
                    x = conv3_layer1(x, [3, 5, 5, 512, 512], 2)
                    x = batch_normalize_3d(x, is_training)
                    x = tf.nn.relu(x)
                    #print('shape of global dis 5 layer out')
                    #print(x.get_shape().as_list())
                with tf.variable_scope('fc'):
                    x = flatten_layer_3d(x)
        ...
        def local_discriminator(x):
            ...
            with tf.variable_scope('local'):
                ...
                with tf.variable_scope('conv2'):
                    ...
                    x = batch_normalize_3d(x, is_training)
                    x = tf.nn.relu(x)
                    #print('shape of local dis 2 layer out')
                    #print(x.get_shape().as_list())
                with tf.variable_scope('conv3'):
                    x = conv3_layer1(x, [3, 5, 5, 128, 256], 2)
                    x = batch_normalize_3d(x, is_training)
                    x = tf.nn.relu(x)
                    #print('shape of local dis 3 layer out')
                    #print(x.get_shape().as_list())
                with tf.variable_scope('conv4'):
                    x = conv3_layer1(x, [3, 5, 5, 256, 512], 2)
                    x = batch_normalize_3d(x, is_training)
                    x = tf.nn.relu(x)
                    #print('shape of local dis 4 layer out')
                    #print(x.get_shape().as_list())
                with tf.variable_scope('fc'):
                    x = flatten_layer_3d(x)
                    x = full_connection_layer(x, 1024)
    ...
    def calc_g_loss(self, x, completion):
        ...
        #g_loss = tf.losses.mean_squared_error(x,completion)
        return g_loss
    ...
    def calc_d_loss(self, real, fake):
        print('input to loss '+str(real.get_shape().as_list()))
